...
import org.jetbrains.kotlin.test.directives.FirDiagnosticsDirectives.JVM_JS_DIFFERENCE
...
class KlibLoadedMetadataDumpHandler(testServices: TestServices) : AbstractLoadedMetadataDumpHandler<BinaryArtifacts.KLib>(
    ...
    override fun prepareSessions(
        ...
        val resolvedLibraries = CommonJsKLibResolver.resolve(libraries, DummyLogger).getFullResolvedList()
...
abstract class AbstractLoadedMetadataDumpHandler<A : ResultingArtifact.Binary<A>>(
    ...
    override fun processAfterAllModules(someAssertionWasFailed: Boolean) {
        ...
        if (JVM_JS_DIFFERENCE in testServices.moduleStructure.allDirectives) {
            ...
            val checks = listOf(commonExtension, specificExtension, otherSpecificExtension).map { extension ->
                {
                    ...
                    assertions.assertFalse(baseFile.exists()) {
                        "Base file $baseFile exists in presence of $JVM_JS_DIFFERENCE directive. Please remove file or directive"
                    }
                }
            }
            ...
            checkDumpsIdentity(
                ...
                postProcessTestData = { it.replace("// $JVM_JS_DIFFERENCE\n", "") }
            )
        } else {
